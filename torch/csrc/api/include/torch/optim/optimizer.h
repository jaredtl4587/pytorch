...
class TORCH_API OptimizerParamGroup {
 public:
  ...
  OptimizerOptionsBase* options() const {
...
class TORCH_API OptimizerBase {
 public:
  OptimizerBase(const OptimizerBase& optimizer_base)
    : parameters_(optimizer_base.parameters_),
      defaults_(optimizer_base.defaults_->clone()),
      param_groups_(optimizer_base.param_groups_) {
    for (const std::pair<at::TensorImpl*, std::unique_ptr<OptimizerParamStateBase>>& elem : optimizer_base.state_) {
      state_[elem.first] = elem.second->clone();
    } 
  }
